...
fft.exe: fft.c; cl fft.c /Ox /G6Fy /MD /DFFTTEST /link /opt:nowin98
	del fft.obj
!if 0
#endif
...
typedef struct
{
	int cbl, curdctblksiz; //current block sizes (clear to 0 before calling fft functions the first time)
	int *bitrev, *bitrev2, *bitrevreal;
	float *sinlut, *coslut, *sinlut2, *coslut2, *radix4tab, oneovern, twoovern;
	float *dctlut, *dctflut, *dctilut, *tempr, *tempi; //for DCT only
	void (*fftfunc)(void *, float *, float *, int);
} fft_t;

	//Real value optimized FFT. dir:1=forward fft,-1=inverse fft
extern void realfft (fft_t *fft, float *snd, float *real, float *imag, int log2blocksiz, int dir);

	//In-place complex-to-complex FFT. dir=1:forward fft, dir=-1:inverse fft
extern void compfft (fft_t *fft, float *real, float *imag, int log2blocksiz, int dir);

	//Fast Discrete Cosine Transform (type 2). dir:1=forward fft,-1=inverse fft
extern void fastdct (fft_t *fft, float *tim, float *frq, int log2blocksiz, int dir);
...
Here's a snippet of C code which shows how to use realfft():
	...
	fft_t fft; memset(&fft,0,sizeof(fft_t));
	for(i=0;i<1024;i++) snd[i] = (float)(rand()-16384);
	realfft(&fft,snd,real,imag,10,1);
...
For dynamically allocated arrays, you can use _aligned_malloc(numbytes,16)/
_aligned_free from MSVC (VC6 + Processor Pack or above), or align it yourself
by allocating 15 extra bytes and using this formula:

	new_pointer = (((int)pointer+15)&~15);

I could make my library handle unaligned arrays, but this would require extra
steps which would slow it down.

For various reasons, you may need to initialize the lookup tables for a
particular block size in advance. To do this, you may call any of the 3
functions (realfft, compfft, fastdct) with a direction value of 0. In this
case, the lookup tables for the passed logblocksiz will be calculated but the
input and output arrays will be ignored. You may set them to null. For
example:
	realfft(&fft,0,0,0,10,1);
...
typedef struct
{
	...
	//Note: If not using compfft, the above array sizes (not counting DCT) can be halved!
...
#define FFTRADIX4ASM 1 //WARNING:Setting this to 0 is not thread-safe: fftradix4odd() overwrites fft->sinlut/fft->coslut temporarily!
__declspec(align(16)) static const int neg01[4] = {0x80000000,0x80000000,0,0};
__declspec(align(16)) static const int neg12[4] = {0,0x80000000,0x80000000,0};
static void fftradix4 (void *vfft, float *r, float *i, int n)
{
#if (FFTRADIX4ASM == 0)
	int i0, i1, i2, i3, pw, a, aa, ainc, msk, pw3;
	float f, g, r1, q1, r2, q2, r3, q3, *nr, *ni;
	fft_t *fft = (fft_t *)vfft;
	...
	while (ainc > 4)
	{
		...
		do
		{
			...
			r1 = fft->coslut[ a]*r[i1] + fft->sinlut[ a]*i[i1];
			q1 = fft->coslut[ a]*i[i1] - fft->sinlut[ a]*r[i1]; aa = a+a;
			r2 = fft->coslut[aa]*r[i2] + fft->sinlut[aa]*i[i2];
			q2 = fft->coslut[aa]*i[i2] - fft->sinlut[aa]*r[i2]; aa = aa+a;
			r3 = fft->coslut[aa]*r[i3] + fft->sinlut[aa]*i[i3];
			q3 = fft->coslut[aa]*i[i3] - fft->sinlut[aa]*r[i3];
...
static void fftradix4odd (void *vfft, float *r, float *i, int nn)
{
#if (FFTRADIX4ASM == 0)
...
#if (FFTRADIX4ASM == 0)
	osinlut = fft->sinlut; fft->sinlut = fft->sinlut2; //WARNING:NOT thread-safe!
	ocoslut = fft->coslut; fft->coslut = fft->coslut2; //WARNING:NOT thread-safe!
#endif
	fftradix4(fft,r,i,nn);
	fftradix4(fft,&r[nn],&i[nn],nn);
#if (FFTRADIX4ASM == 0)
	fft->sinlut = osinlut; fft->coslut = ocoslut;
#endif
	...
	{
		j = z+nn;
		f = r[j]*fft->coslut2[z] + i[j]*fft->sinlut2[z];
		g = i[j]*fft->coslut2[z] - r[j]*fft->sinlut2[z];
		r[j] = r[z]-f; r[z] += f;
		i[j] = i[z]-g; i[z] += g;
	}
#else
	_asm
	{
		...
		mov edi, i
		mov eax, fft
		mov ebx, [eax + fft_t.sinlut2]
		mov eax, [eax + fft_t.coslut2]
		lea edx, [ecx+ecx-4]
		sub ecx, 4
...
static __inline void fcalcsinasm (float *sintabptr, int num, double ai)
{
...
static void initffttables (fft_t *fft, int m)
{
	int i, j, k, n, no1, no2, no3, a, aa, ainc, pw;
	float *fptr;

	if ((fft->cbl == m) || (m <= 1)) return;
	n = (1<<m); fft->cbl = m; no1 = (n>>1); no2 = (n>>2); no3 = (n>>3);
	fft->oneovern = 1.f / (float)n; fft->twoovern = fft->oneovern+fft->oneovern;

	if (fft->radix4tab) { _aligned_free(fft->radix4tab); fft->radix4tab = 0; }
	if (fft->bitrev2) { _aligned_free(fft->bitrev2); fft->bitrev2 = 0; }
	if (fft->bitrev) { _aligned_free(fft->bitrev); fft->bitrev = 0; }
	if (fft->sinlut) { _aligned_free(fft->sinlut); fft->sinlut = 0; }
	i = n+no2; if (i < 16) i = 16;
	fft->sinlut = (float *)_aligned_malloc(i*sizeof(float),16); if (!fft->sinlut) { fft->cbl = 0; return; }
	fft->bitrev = (int *)_aligned_malloc(no1*sizeof(int),16); if (!fft->bitrev) { fft->cbl = 0; return; }
#if (FFTTEST == 1)
	if (lb0 == lb1)
	{
	...
	//for(i=0;i<=no2;i++) fft->sinlut[i] = sin((double)i*PI*2.0/(float)n);
	fcalcsinasm(fft->sinlut,no2+1,fft->oneovern);
	for(i=no2+1;i<no1;i++) fft->sinlut[i] = fft->sinlut[no1-i];
	for(i=no1;i<n;i++) fft->sinlut[i] = -fft->sinlut[i-no1];
	if (no2 >= 4)
	{
		fft->coslut = &fft->sinlut[no2];
		for(i=0;i<no2;i++) fft->sinlut[i+n] = fft->sinlut[i];
	}
	else //This ensures coslut is 16-byte aligned (for very small block sizes) :/
	{
		fft->coslut = &fft->sinlut[n];
		for(i=n-1;i>=0;i--) fft->coslut[i] = fft->sinlut[(i+no2)&(n-1)];
	}
	...
	if (m&1)
	{
		if (n >= 32)
		{
			...
			fft->radix4tab = (float *)_aligned_malloc(j*24*sizeof(float),16); if (!fft->radix4tab) { fft->cbl = 0; return; }
#if (FFTTEST == 1)
			if (lb0 == lb1) printf("radix4tab: %d bytes\n",j*24*sizeof(float));
#endif
			ainc = no2; fptr = fft->radix4tab;
			for(ainc=(no2>>2),pw=1;ainc>1;ainc>>=2,pw<<=2)
				for(i=(ainc<<2),aa=no2-i;aa>=0;aa-=i,fptr+=24)
					for(a=aa,k=0;k<4;k++)
					{
						fptr[k+ 0] = fft->coslut[a]; fptr[k+ 4] = fft->sinlut[a]; j = a+a;
						fptr[k+ 8] = fft->coslut[j]; fptr[k+12] = fft->sinlut[j]; j += a;
						fptr[k+16] = fft->coslut[j]; fptr[k+20] = fft->sinlut[j]; a += ainc;
					}
		}

		fft->fftfunc = fftradix4;
		for(j=1,k=no3;k>0;j<<=2,k>>=2)
			for(i=0;i<j*(4-1);i++) fft->bitrev[i+j] = fft->bitrev[i]+k;

		fft->bitrevreal = fft->bitrev;
	}
	else
	{
		if (fft->sinlut2) { _aligned_free(fft->sinlut2); fft->sinlut2 = 0; }
		i = no1+no3; if (i < 16) i = 16;
		fft->sinlut2 = (float *)_aligned_malloc(i*sizeof(float),16); if (!fft->sinlut2) { fft->cbl = 0; return; }
#if (FFTTEST == 1)
		if (lb0 == lb1) printf("  sinlut2: %d bytes\n",i*sizeof(float));
#endif
		if (no3 >= 4)
		{
			fft->coslut2 = &fft->sinlut2[no3];
			for(i=no1+no3-1;i>=0;i--) fft->sinlut2[i] = fft->sinlut[i+i];
		}
		else //This ensures coslut2 is 16-byte aligned (for very small block sizes) :/
		{
			fft->coslut2 = &fft->sinlut2[no1];
			for(i=no1-1;i>=0;i--) { fft->sinlut2[i] = fft->sinlut[i+i]; fft->coslut2[i] = fft->coslut[i+i]; }
		}
		...
		if (n >= 64)
		{
			...
			if (lb0 == lb1) printf("radix4tab: %d bytes\n",j*24*sizeof(float));
#endif
			ainc = no3; fptr = fft->radix4tab;
			for(ainc=(no3>>2),pw=1;ainc>1;ainc>>=2,pw<<=2)
				for(i=(ainc<<2),aa=no3-i;aa>=0;aa-=i,fptr+=24)
					for(a=aa,k=0;k<4;k++)
					{
						fptr[k+ 0] = fft->coslut2[a]; fptr[k+ 4] = fft->sinlut2[a]; j = a+a;
						fptr[k+ 8] = fft->coslut2[j]; fptr[k+12] = fft->sinlut2[j]; j += a;
						fptr[k+16] = fft->coslut2[j]; fptr[k+20] = fft->sinlut2[j]; a += ainc;
					}
		}

		fft->fftfunc = fftradix4odd;
		for(j=1,k=no2;k>0;j<<=1,k>>=1)
			for(i=0;i<j*(2-1);i++) fft->bitrev[i+j] = fft->bitrev[i]+k;

		fft->bitrev2 = (int *)_aligned_malloc(no1*sizeof(int),16); if (!fft->bitrev2) { fft->cbl = 0; return; }
#if (FFTTEST == 1)
		if (lb0 == lb1) printf("  bitrev2: %d bytes\n",no1*sizeof(int));
#endif
		for(i=no2-1;i>=0;i--)
		{
			j = fft->bitrev[((i&0xaaaaaaaa)>>1) + ((i&0x55555555)<<1)];
			fft->bitrev2[i    ] = j;
			fft->bitrev2[i+no2] = j+1;
		}
		fft->bitrevreal = fft->bitrev2;
	}
}

#define WASTE_32K_TO_SPEEDUP_COMPFFT 0 //WARNING:Setting this to 1 not thread-safe!
...
static float oldr[8192], oldi[8192];
	 ...
	 //   m: log2(n)
	 //   r/i: real/imag
void compfft (fft_t *fft, float *r, float *i, int m, int dir)
{
	float f, g, h;
	int j, n, z;
	...
	initffttables(fft,m+1); if (!dir) return;
	n = (1<<m);
	...
	if (n <= (sizeof(oldr)/sizeof(oldr[0])))
	{
		...
		if (dir > 0)
		{
			for(z=n-1;z>=0;z--) { j = fft->bitrevreal[z]; r[z] = oldr[j]; i[z] = oldi[j]; }
		}
		else
		{
			g = fft->twoovern; h = -g;
			for(z=n-1;z>=0;z--) { j = fft->bitrevreal[z]; r[z] = oldr[j]*g; i[z] = oldi[j]*h; }
		}
	}
	else
#endif
...
__declspec(align(16)) static const float onehalf[4] = {.5f,.5f,.5f,.5f};
void realfft (fft_t *fft, float *snd, float *r, float *i, int m, int dir)
{
	...
	initffttables(fft,m); if (!dir) return;
...
#if 0
		for(z=bo2-1;z>=0;z--) { j = (fft->bitrevreal[z]<<1); r[z] = snd[j]; i[z] = snd[j+1]; }
#else
		//for(z=0;z<bo2;z++) printf("%d ",fft->bitrevreal[z]);
...
begr0:   mov eax, [ebp+ebx*4]
		...
		do
		{
			...
			r[z] = (re + ro*fft->coslut[z] + io*fft->sinlut[z])*.5f;
			i[z] = (ie + io*fft->coslut[z] - ro*fft->sinlut[z])*.5f;
			r[zz] = re - r[z];
			i[zz] = i[z] - ie;
			z++; zz--;
		} while (z < zz);
		...
		_asm
		{
			...
			mov eax, fft
			mov ebx, [eax + fft_t.sinlut]
			mov eax, [eax + fft_t.coslut]
...
begrf:
	...
	else
	{
		...
		do
		{
			...
			f = ro*fft->coslut[zz] + io*fft->sinlut[zz];
			g = io*fft->coslut[zz] - ro*fft->sinlut[zz];
		...
		_asm
		{
			...
			mov ebp, bo2    ;(0)1 2 3 4  5 6 7 88 9 10 11  12 13 14 15(16)
			sub edi, esi               ;edi: i-r
			sub eax, esi               ;eax: coslut-r
			sub ebx, esi               ;ebx: sinlut-r
			sub ecx, esi               ;ecx: snd-r
			sub edx, esi               ;edx: sndi-r
			lea ebp, [ebp*4+esi-16]    ;ebp: (zz<<2)+r
			add esi, 4                 ;esi: (z<<2)+r
begri:
			movups xmm0, [edi+esi]     ;xmm0: iz (backwards)
			movaps xmm1, [ebp]         ;xmm1: rzz
			movups xmm2, [esi]         ;xmm2: rz (backwards)
			movaps xmm3, [edi+ebp]     ;xmm3: izz
			...
			mulps xmm0, [eax+ebp]      ;xmm0: ro*cos
			mulps xmm5, [ebx+ebp]      ;xmm5: io*sin
			mulps xmm4, [ebx+ebp]      ;xmm4: ro*sin
			mulps xmm1, [eax+ebp]      ;xmm1: io*cos
...
#if 0
		for(z=bo2-1;z>=0;z--) { j = fft->bitrevreal[z]; r[z] = snd[j]; i[z] = sndi[j]; }
#else
		_asm
		{
			push ebx
...
#if 0
		for(z=0;z<bo2;z++) { snd[z+z] = r[z]*fft->oneovern; snd[z+z+1] = i[z]*fft->oneovern; }
#else
		_asm
		{
			push edi
			mov eax, fft
			movss xmm7, [eax + fft_t.oneovern]
			mov eax, n
			mov ecx, r
			add eax, eax
			mov edi, i
...
static void initdcttables (fft_t *fft, int n)
{
	float c, s;
	int i, no2;

	if (n == fft->curdctblksiz) return;
	fft->curdctblksiz = n;

	n = (1<<n); no2 = (n>>1);
	if (fft->dctlut) { _aligned_free(fft->dctlut); fft->dctlut = 0; }
	i = (((n>>1)+3)&~3)*3 + (((n>>1)+1+3)&~3)*2;
	fft->dctlut = (float *)_aligned_malloc(i*sizeof(float),16);
	if (!fft->dctlut) { fft->curdctblksiz = 0; return; }
#if (FFTTEST == 1)
	if (lb0 == lb1) printf("   dctlut: %d bytes\n",i*sizeof(float));
#endif
	fft->dctflut = &fft->dctlut[((n>>1)+3)&~3];
	fft->dctilut = &fft->dctflut[((n>>1)+3)&~3];
	fft->tempr = &fft->dctilut[((n>>1)+3)&~3];
	fft->tempi = &fft->tempr[((n>>1)+1+3)&~3];

	fcalcsinasm(fft->dctflut,n,0.25/(float)n); //This is an ugly memory hack... but it IS safe!
	for(i=0;i<no2;i++) fft->dctlut[i] = fft->dctflut[i+i];     //WARNING: Can't fuse loop!
	for(i=0;i<no2;i++) fft->dctflut[i] = fft->dctflut[i+i+1];  //WARNING: Can't fuse loop!
	for(i=0;i<no2;i++) fft->dctilut[i] = .25f/fft->dctflut[i]; //WARNING: Can't fuse loop!
}

	//   //slowdct is equivalent to fastdct, but MUCH SLOWER!
	//void slowdct (float *x, float *y, int logn, int dir)
	//{  int j, k, n; float f, g, s; n = (1<<logn); f = PI / (float)n;
	//   if (dir > 0)
	...
	//}  }
void fastdct (fft_t *fft, float *snd, float *y, int logn, int dir)
{
	int i, j, m, n, no2;
	float a, b, c, d;
	...
	initdcttables(fft,logn); if (!dir) return;
...
#endif
		realfft(fft,y,fft->tempr,fft->tempi,logn,1);
		y[n-1] = fft->tempr[no2]*.5f;
		for(i=no2-1;i;i--)
		{
			j = i+i;
			y[j  ] = fft->dctlut[no2-i]*fft->tempr[i] + fft->dctlut[    i]*fft->tempi[i];
			y[j-1] = fft->dctlut[    i]*fft->tempr[i] - fft->dctlut[no2-i]*fft->tempi[i] + y[j+1];
		}
		y[0] = fft->tempr[0];
	}
	else
	{
		fft->tempr[no2] = y[n-1]*2; fft->tempi[no2] = 0;
		for(i=no2-1;i;i--)
		{
			j = i+i; a = y[j-1]-y[j+1];
			fft->tempr[i] = fft->dctlut[no2-i]*y[j] + fft->dctlut[    i]*a;
			fft->tempi[i] = fft->dctlut[    i]*y[j] - fft->dctlut[no2-i]*a;
		}
		fft->tempr[0] = y[0]; fft->tempi[0] = 0;
		realfft(fft,snd,fft->tempr,fft->tempi,logn,-1);
#if 0
		for(i=no2-1;i>=0;i--)
		{
			a = (snd[i]+snd[m-i])*.5f;
			b = (snd[i]-snd[m-i])*fft->dctilut[i];
...
#if 0
float getentropybits (int *frqbuf, int num)
{
	double d;
	int z, t;
...
void main (int argc, char **argv)
{
	...
	int i, lblocksiz, blocksiz, compbytes, testmode;
	...
	for(lblocksiz=lb0;lblocksiz<=lb1;lblocksiz++)
	{
		...
		snd = (float *)_aligned_malloc(blocksiz*sizeof(float),16); if (!snd) { puts("malloc 1 failed"); exit(0); }
		switch(testmode)
		{
			case 0: case 2: compbytes = blocksiz*sizeof(float); break;
			case 1: compbytes = ((blocksiz>>1)+1)*sizeof(float); break;
		}
		real = (float *)_aligned_malloc(compbytes,16); if (!real) { puts("malloc 2 failed"); exit(0); }
		imag = (float *)_aligned_malloc(compbytes,16); if (!imag) { puts("malloc 3 failed"); exit(0); }
		oreal = (float *)_aligned_malloc(compbytes,16); if (!oreal) { puts("malloc 4 failed"); exit(0); }
		oimag = (float *)_aligned_malloc(compbytes,16); if (!oimag) { puts("malloc 5 failed"); exit(0); }
		snd2 = (float *)_aligned_malloc(blocksiz*sizeof(float),16); if (!snd2) { puts("malloc 6 failed"); exit(0); }
		if (lb0 == lb1)
		{
		...
		{
			case 0:
				compfft(&fft,0,0,lblocksiz,0); //Hack to make table calculation not count in timings
				memcpy(real,snd,blocksiz*sizeof(float)); //remember to allocate real&imag at full size!
				memset(imag,0,blocksiz*sizeof(float));
				if (lb0 == lb1) { q0 = rdtsc64(); compfft(&fft,real,imag,lblocksiz,1); q1 = rdtsc64(); bestq = q1-q0; }
				else
				{
					...
					for(i=0;i<32;i++)
					{
						...
						q0 = rdtsc64(); compfft(&fft,real,imag,lblocksiz,1); q1 = rdtsc64();
				...
				break;
			case 1:
				realfft(&fft,0,0,0,lblocksiz,0); //Hack to make table calculation not count in timings
				if (lb0 == lb1)
				{
					q0 = rdtsc64(); realfft(&fft,snd,real,imag,lblocksiz,1); q1 = rdtsc64(); bestq = q1-q0;
				}
				else
				{
					for(i=0;i<32;i++)
					{
						q0 = rdtsc64(); realfft(&fft,snd,real,imag,lblocksiz,1); q1 = rdtsc64();
						if (q1-q0 < bestq) bestq = q1-q0;
					}
				}
				...
				break;
			case 2:
				fastdct(&fft,0,0,lblocksiz,0); //Hack to make table calculation not count in timings
				if (lb0 == lb1) { q0 = rdtsc64(); fastdct(&fft,snd,real,lblocksiz,1); q1 = rdtsc64(); bestq = q1-q0; }
				else
				{
					for(i=0;i<32;i++)
					{
						q0 = rdtsc64(); fastdct(&fft,snd,real,lblocksiz,1); q1 = rdtsc64();
						if (q1-q0 < bestq) bestq = q1-q0;
					}
				}
		...
		if (fft.fftfunc == fftradix4) printf("radix4");
										 else printf("radixM");
		...
		switch(testmode)
		{
			...
			case 1:
				realfft(&fft,0,0,0,lblocksiz,0); //Hack to make table calculation not count in timings
				if (lb0 == lb1) { q0 = rdtsc64(); realfft(&fft,snd2,real,imag,lblocksiz,-1); q1 = rdtsc64(); bestq = q1-q0; }
				else
				{
					memcpy(oreal,real,compbytes); memcpy(oimag,imag,compbytes);
					for(i=0;i<32;i++)
					{
						memcpy(real,oreal,compbytes); memcpy(imag,oimag,compbytes);
						q0 = rdtsc64(); realfft(&fft,snd2,real,imag,lblocksiz,-1); q1 = rdtsc64();
						if (q1-q0 < bestq) bestq = q1-q0;
					}
				}
			...
			case 2:
				fastdct(&fft,0,0,lblocksiz,0); //Hack to make table calculation not count in timings
				if (lb0 == lb1) { q0 = rdtsc64(); fastdct(&fft,snd2,real,lblocksiz,-1); q1 = rdtsc64(); bestq = q1-q0; }
				else
				{
					for(i=0;i<32;i++)
					{
						q0 = rdtsc64(); fastdct(&fft,snd2,real,lblocksiz,-1); q1 = rdtsc64();
						if (q1-q0 < bestq) bestq = q1-q0;
					}
				}
		...
		_aligned_free(snd2); _aligned_free(oimag); _aligned_free(oreal); _aligned_free(imag); _aligned_free(real); _aligned_free(snd);
		if (fft.dctlut) { _aligned_free(fft.dctlut); fft.dctlut = 0; }
		if (fft.bitrev2) { _aligned_free(fft.bitrev2); fft.bitrev2 = 0; }
		if (fft.radix4tab) { _aligned_free(fft.radix4tab); fft.radix4tab = 0; }
		if (fft.bitrev) { _aligned_free(fft.bitrev); fft.bitrev = 0; }
